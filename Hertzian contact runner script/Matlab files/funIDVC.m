...
% This passes all the varargin arguments of the outer function funIDVC to
% parseInputs, expanding the array into a comma-separated list to pass
% individual arguments. Here: filename, sSize, incORcum
I{1} = loadFile(fileInfo,1);


%% ---- Opening and Reading Subsequent Images ---
numImages = length(fileInfo.filename);   % Reads how many files were found under filename
...
%Test whether the file name is correct:
...
if iscell(filename)
    % If input is cell array: use it directly
    fileInfo.filename = filename;
    fileInfo.datachannel = 1;
else
    % If input is string: use wildcard matching
    [~,filenameNoExt,~] = fileparts(filename);
    files = dir([filenameNoExt, '.mat']);
    
    if isempty(files)
        error('No files found matching: %s.mat', filenameNoExt);
    end

    fileInfo.filename = {files.name};
    fileInfo.datachannel = 1;
end
...
% Ensure subset size is a multiple of 32
