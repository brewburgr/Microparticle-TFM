...
disp('Starting addDisplacements')

% Writing out the for loop and adding a 0, NaN, and non-zero numbers check
% before and after inpainting. Original in the line below:
%for i = 1:3, du0{i} = inpaint_nans3(du0{i}); end % remove NaNs if presen

for i = 1:3
    % Check before inpainting
    fprintf('Before inpainting for du0{%d}:\n', i);
    
    if all(isnan(du0{i}(:)))
        fprintf('  All entries are NaN.\n');
    elseif all(du0{i}(:) == 0)
        fprintf('  All entries are zero.\n');
    else
        fprintf('  du0{%d} contains mixed values (non-zero/non-NaN).\n', i);
    end
    
    if any(isnan(du0{i}(:)))
        fprintf('  Contains NaN values.\n');
    else
        fprintf('  Does not contain NaN values.\n');
    end

    % Check for numbers that are not 0 and not NaN
    if any(~isnan(du0{i}(:)) & du0{i}(:) ~= 0)
        fprintf('  Contains numbers that are not 0 and not NaN.\n');
    else
        fprintf('  Does not contain any numbers that are not 0 and not NaN.\n');
    end

    % Apply inpainting
    du0{i} = inpaint_nans3(du0{i});

    % Check after inpainting
    fprintf('After inpainting for du0{%d}:\n', i);
...
disp('Finished addDisplacements')
end