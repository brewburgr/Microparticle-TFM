...
% PRESET CONSTANTS
maxIterations = 20; % maximum number of iterations, originally 20
...
%disp(I0);
%disp(I0{1})
isAllZeros = all(cellfun(@(x) isequal(x, 0), I0(:))); % Check if all entries are 0
disp(isAllZeros)
...
% Stop after either convergence has been met or at max iterations
while ~converged01 && i - 1 < maxIterations
    ...
    if ~converged01
        ...
        disp('Checking inputs of DVC:')

        isAllZeros = all(cellfun(@(x) isequal(x, 0), I(:))); % Check if all entries are 0
        disp(isAllZeros)
        %return

        [du, cc] = DVC(I,sSize(i,:),sSpacing(i,:),DVCPadSize,ccThreshold);

        disp('One DVC step done')
        ...
        % return % No crash if this return is active
...
% ---
% The following two lines are fine for Matlab 2023b, but Matlab 2025b need
% the updated version below for z dimensions that do not match with the
% sSize:
%idx = cell(1,3);
...
idx = cell(1,3);
for i = 1:3
    % Ensure sSpacing(i) and sizeI(i) are valid real scalars
    if isempty(sizeI(i)) || ~isreal(sizeI(i)) || ~isscalar(sizeI(i))
        error('parseImages:BadSizeI','sizeI(%d) invalid: %s', i, mat2str(sizeI(i)));
    end
    if isempty(sSpacing(i)) || ~isreal(sSpacing(i)) || ~isscalar(sSpacing(i))
        error('parseImages:BadSpacing','sSpacing(%d) invalid: %s', i, mat2str(sSpacing(i)));
    end

    % Round step size to ensure colon operator works
    step = max(1, round(sSpacing(i)));

    % Build index vector and shift by half subset size
    idx{i} = 1:step:(sizeI(i)+1);
    idx{i} = idx{i} + sSize(i)/2;
end
% ---
