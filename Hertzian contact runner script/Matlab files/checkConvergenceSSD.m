...
% Calculate the SSE (see eq. 12)
I{1} = I{1}(:); I{2} = I{2}(:); % Reshape data in I{1} and I{2} to column vectors
N = numel(I{1});

% Check if they have the same size
if numel(I{1}) ~= numel(I{2})
    error('I{1} and I{2} must have the same number of elements');
end
...
SSE(end + 1) = SSE1; % Put the errors in an array
...
if iteration > 1 % skip before first displacement estimation
    ...
    % Check for convergence before refining window size:
...
% Stop FIDVC if convergence has been met:
