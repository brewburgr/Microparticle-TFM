...
We provide a [runner script](./Hertzian%20contact%20runner%20script) for an exemplary simulated Hertzian contact scenario (applying both the volume and surface methods), together with the Matlab files for the FIDVC, and example data and results.

We also include a [JupyterLab notebook](./Jupyter%20notebook%20experimental%20+%20template) applying the surface method to our experimental data from DNA hydrogel microparticles (DNA-HMPs) compressed from the top.
...
**ShElastic package** (Python functions for spherical harmonics, used in our scripts):
